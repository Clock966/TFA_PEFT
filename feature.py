...
class ConvSTFT(nn.Module):
    ...
    def __init__(self,
        ...
        # self.weight = nn.Parameter(kernel, requires_grad=True)    # (not fix))
...
class ConviSTFT(nn.Module):
    ...
    def forward(self, inputs, phase, cplx=False):
        ...
        # N x 1 x L
        w=self.weight
        outputs = F.conv_transpose1d(cspec, w, stride=self.stride)
...
def test_stft():
    ...
    if single:
        x = th.randn([3, 16000 * 4])        # B L
        mag, phase = stft(x, cplx=False)
        print('mag.shape: ', mag.shape)
        print('phase.shape: ', phase.shape)

    else:
        x = th.randn([3, 1, 16000 * 4])     # B C L
        mag, phase = stft(x)
        mag = mag.squeeze(1)
